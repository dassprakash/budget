...
<body ng-app="monthlyApp" ng-controller="budjetController">
...
<div id="wrapper">
	...
	<div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><?php echo 'Budjet Reports';?></h1>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        <!-- /.row -->
        
       <a class="btn white btn-success " ng-click="openmodal(add)"><i class="fa fa-plus"></i> <?php echo 'New Transaction'; ?></a>
       
        <div class="row">
			<div class="col-lg-12">
                <div class="panel panel-primary">
                    <div class="panel-heading">
                       <i class="glyphicon glyphicon-stats"></i> <?php echo 'This month Budjet';?>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="">
                            <table class="table table-bordered table-hover table-striped" id="assetsdata">
        	                	...
        	                	<tbody>        							 
        							<tr ng-repeat="(key,value) in Budjetlist">
        							<td>{{value.Title}}</td>
        							<td>{{value.Dates}}</td>
        							<td>{{value.CategoryName}}</td>
        							<td>{{value.AccountName}}</td>
        							<td>   
													<span style="cursor:default;" class="btn btn-danger btn-circle" ng-if="value.status=='N'" ><i class="fa fa-times"></i></span>
													<span style="cursor:default;" class="btn btn-success btn-circle" ng-if="value.status=='Y'"><i class="fa fa-check"></i></span>
											<a ng-if="value.status=='N'"  data-toggle="modal" ng-click="change_status(value.BudjetId)"><span class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="right" title="" data-original-title="<?php echo 'paid budget' ;?>">change</span></button>		
											</td>
        							<td>Rs:{{value.Amount}}</td>
        							<td colspan="2" class="notification">
        								<a href="#EditIncome" class="" data-toggle="modal" ng-click="editmodel(value.Title,value.Amount,value.CategoryId,value.AccountId,value.Dates,value.Description,value.AccountName,value.BudjetId)"><span class="btn btn-primary btn-xs glyphicon glyphicon-edit" data-toggle="tooltip" data-placement="left" title="" data-original-title="<?php echo 'Edit Budjet';?>"></span></a>
        								<a href="#DeleteIncome"  data-toggle="modal" ng-click="deletemodel(value.BudjetId)"><span class=" glyphicon glyphicon-trash btn btn-primary btn-xs" data-toggle="tooltip" data-placement="right" title="" data-original-title="<?php echo 'Delete Incomes' ;?>"></span></button>		
        							</td>
        							</tr>
											<tr ng-if="Budjetlist.length==0">
											<td colspan="7" align="center"><h4>Sorry No records Found</h4></td>
											</tr>
        	                	</tbody>			
...
<div class="modal fade" id="EditIncome" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">    
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="myModalLabel">Edit Budject</h4>
            </div>
            <div class="modal-body">
                <fieldset>
            ...
            <div class="modal-footer">
                ...
                <button type="input" id="submit" name="submitin" class="btn btn-primary" ng-click="editincome(AssetsId)" ng-show="!add"><?php echo 'Yes' ;?></button>
                <button type="input" id="submit" name="submitin" class="btn btn-primary" ng-click="addbudject()" ng-show="add"><?php echo 'Yes' ;?></button>
                <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo 'Cancel' ;?></button>
...
<script type="text/javascript" src="js/angular_controller/budjet_controller.js"></script> 
